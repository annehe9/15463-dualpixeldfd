...
reference = io.imread(fullpath+"/reference.png")
...
confidence = np.ones(confidence.shape)
...
io.imsave(fullpath+'/bilateral_solver_result.png', img_as_ubyte(output_solver))

reference = (reference*255).astype(np.uint8)
output_solver = (output_solver*255).astype(np.uint8)
q = guidedFilter(reference, output_solver, radius=2, eps=1e-6)
plt.imsave(fullpath+'/output.png', q, cmap='viridis')
